...
export const GLOBAL_CHART = gql`
  query MarsDayDatas($startTime: Int!, $skip: Int!) {
      marsDayDatas(first: 1000, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
      id
      date
      totalVolumeUSD
      dailyVolumeUSD
...
export const GLOBAL_DATA = (block) => {
  const queryString = ` query marsFactories {
      marsFactories(
...
export const ALL_PAIRS = (block) => {
  ...
  query pairs($skip: Int!) {
    pairs(first: 100, skip: $skip, orderBy: trackedReserveBNB, orderDirection: desc, where: { block: 5753123 }) {
      id
      token0 {
        id
        symbol
...
export const PAIRS_CURRENT = gql`
  query pairs {
    pairs(first: 100, where: { block : 5753123 },  orderBy: trackedReserveBNB, orderDirection: desc) {
      id
    }
  }
`
